...
namespace Contract.API.Controllers
{
    ...
    /// </summary>
    [RoutePrefix("sign-of-user")]
    public class SignOfUserController : BaseController
    {
        #region Fields, Properties

        private static readonly Logger logger = new Logger();
        private readonly SignOfUserBusiness business;

        #endregion // #region Fields, Properties

        #region Contructor

        public SignOfUserController()
        {
            business = new SignOfUserBusiness(GetBOFactory());
        }
        ...
        {
            var response = new ApiResultList<SignOfUserInfo>();
            try
            {
        ...
        public IHttpActionResult GetDetail(int id)
        {
            ...
            var response = new ApiResult<SignOfUserInfo>();
            try
            {
                response.Code = ResultCode.NoError;
                response.Message = MsgApiResponse.ExecuteSeccessful;
        ...
        public IHttpActionResult Create(SignOfUserInfo productInfo)
        {
            if (productInfo == null)
            {
                return Error(ResultCode.DataInvalid, MsgApiResponse.DataInvalid);
        ...
        public IHttpActionResult Update(int id, SignOfUserInfo productInfo)
        {
        ...
        [HttpPost]
        [Route("use-default/{id}")]
        //[CustomAuthorize(Roles = UserPermission.CompanyManagement_Update)]
        public IHttpActionResult UpdateUseDefault(int id)
        {
            var response = new ApiResult();

            try
            {
                response.Code = this.business.UpdateUseDefault(id);
                response.Message = MsgApiResponse.ExecuteSeccessful;
            }
            catch (BusinessLogicException ex)
            {
