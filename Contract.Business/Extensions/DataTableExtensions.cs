...
namespace Contract.Business.Extensions
{
    public static class DataTableExtensions
    {
        ...
        public static DataTable RemoveRowSpace(this DataTable table)
        {
            ...
            var dataOfDataTable = table.Rows.Cast<DataRow>().Where(row => !row.ItemArray.All(field => field is System.DBNull || !field.IsNullOrEmpty() || string.Compare((field.EmptyNull()).Trim(), string.Empty) == 0));
            if (dataOfDataTable.Count() == 0)
            {
                return null;
            }
